...
export type TaxProfileId = "2026_current" | "2026_legacy";
...
interface TaxConfig {
  ...
  // Sozialversicherung
  rvBeitragssatz: number;
  rvBeitragsbemessungsgrenze: number;
  avBeitragssatz: number;
  avBeitragsbemessungsgrenze: number;
  kvBeitragssatz: number;
  kvBeitragsbemessungsgrenze: number;
  pvBeitragssatz: number;
  pvBeitragsbemessungsgrenze: number;
  pvArbeitnehmerAnteilBasis: number;
  pvKinderlosenzuschlag: number;
  pvKinderabschlag: number;

  // Soli
  soliFreigrenze: number;
  soliMinderungssatz: number;
  soliSatz: number;

  // Lohnsteuer-Details
  vorsorgepauschaleRundung: number;
  stkl5Grenze1: number;
  stkl5Grenze2: number;
  stkl5Grenze3: number;
  avVorsorgeFaktor: number;
}
...
const DEFAULT_TAX_PROFILE: TaxProfileId = "2026_current";

const TAX_PROFILES: Record<TaxProfileId, TaxConfig> = {
...
export function getTaxProfiles(): { id: TaxProfileId; year: number; label: string; description: string }[] {
  return (Object.keys(TAX_PROFILES) as TaxProfileId[]).map((id) => ({
    id,
...
export function getTaxProfileConfig(profileId?: TaxProfileId): TaxConfig {
 ...
 */
function calculateTarif(zve: number, taxConfig: TaxConfig): number {
 ...
 */
function calculateTarifClassFiveSix(zve: number, taxConfig: TaxConfig): number {
...
function calculateUp56(zx: number, taxConfig: TaxConfig): number {
 ...
 */
function calculateSplittingTarif(zve: number, taxConfig: TaxConfig): number {
 ...
 */
function roundTo36(zve: number, taxConfig: TaxConfig): number {
 ...
 */
function calculateSoli(lohnsteuer: number, taxConfig: TaxConfig): number {
 ...
 */
function calculatePVSatz(alter: number, kinderAnzahl: number, taxConfig: TaxConfig): number {
 ...
 */
export function generateProgressionData(taxProfileId?: TaxProfileId): { einkommen: number; grenzsteuersatz: number; durchschnittssteuersatz: number }[] {
